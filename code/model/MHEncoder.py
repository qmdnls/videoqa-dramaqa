...
class UtilityBlock(nn.Module):
    ...
    def __init__(self, hidden_dim, feedforward_dim=2048, nhead=5, dropout=0.1):
        ...
        self.mh_self = nn.MultiheadAttention(hidden_dim, nhead, dropout=dropout) # dropout? separate attention modules?
        self.mh_a = nn.MultiheadAttention(hidden_dim, nhead, dropout=dropout) # dropout? separate attention modules?
        self.mh_b = nn.MultiheadAttention(hidden_dim, nhead, dropout=dropout) # dropout? separate attention modules?
        self.mh_c = nn.MultiheadAttention(hidden_dim, nhead, dropout=dropout) # dropout? separate attention modules?
        #self.multihead_attn = MHA(hidden_dim, pad_size=150, nhead=nhead)
    ...
    def forward(self, target, source_a, source_b, source_c):
        ...
        out_t, _ = self.mh_self(target, target, target) # self attention for target utility
        out_a, _ = self.mh_a(target, source_a, source_a) # attention to source utility a
        out_b, _ = self.mh_b(target, source_b, source_b) # attention to source utility b
        out_c, _ = self.mh_c(target, source_c, source_c) # attention to source utility c
...
class UtilityLayer(nn.Module):
    ...
    Args:
        ...
        dropout: the dropout probability (default=0.1).
    """
    def __init__(self, hidden_dim, feedforward_dim=2048, nhead=6, dropout=0.5):
...
class MHEncoder(nn.Module):
    def __init__(self, args, vocab, n_dim, image_dim, layers, dropout, num_choice=5):
        ...
        self.lstm_raw = RNNEncoder(300, 150, bidirectional=True, dropout_p=0.5, n_layers=2, rnn_type="lstm")

        self.lstm_script = RNNEncoder(321, 150, bidirectional=True, dropout_p=0.5, n_layers=2, rnn_type="lstm")
        ...
        if self.vmeta_on:            
            self.lstm_vmeta = RNNEncoder(321, 150, bidirectional=True, dropout_p=0.5, n_layers=2, rnn_type="lstm")
            self.classifier_vmeta = nn.Sequential(nn.Linear(n_dim*2, 1), nn.Softmax(dim=1))
            self.mhattn_vmeta = CharMatching(3, D, D, dropout=0.5)

        if self.vbb_on:
            self.lstm_vbb = RNNEncoder(image_dim+21, 150, bidirectional=True, dropout_p=0.5, n_layers=2, rnn_type="lstm")
