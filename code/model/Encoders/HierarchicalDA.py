...
class HierarchicalDA(nn.Module):
    def __init__(self, args, vocab, n_dim, image_dim, layers, dropout, num_choice=5):
        ...
        self.util = UtilityLayer(hidden_dim=300, feedforward_dim=1024, n_head=10, dropout=0.1)
        self.util2 = UtilityLayer(hidden_dim=300, feedforward_dim=1024, n_head=10, dropout=0.1)
        self.util3 = UtilityLayer(hidden_dim=300, feedforward_dim=1024, n_head=10, dropout=0.1)
    ...
    def forward(self, que, answers, **features):
        ...
        for feature in features.keys():
            try:    
                print("Name:", feature, "Size:", features[feature].size())
            except:
                print("Name:", feature, "List length:", len(features[feature]))
        ...
        # extract full image features
        frames = self.image_projection(features['filtered_image'])
        ...
        S, _ = self.mh_script(S, Q, Q)
        B, _ = self.mh_bb(B, Q, Q)
        M, _ = self.mh_meta(M, Q, Q)
        #answer_list = [answer.permute(1,0,2) for answer in e_ans_list]
        #A_list = [] 
        #for answer in answer_list:
        #    out, _ = self.mh_answers(answer, Q, Q) 
        #    A_list.append(out)
