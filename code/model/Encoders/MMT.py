...
class MMTEncoder(nn.Module):
    ...
    def forward(self, que, answers, **features):
        ...
        #tgt_len = e_target.size(1)
        #tgt_mask = self._generate_square_subsequent_mask(tgt_len).cuda()

        pad = torch.ones(batch_size, 1, 300).cuda()
        inpt = torch.cat([pad,S,M,B,Q], dim=1)
        
        # sequence-length first
        inpt = inpt.permute(1,0,2)
        #e_target = e_target.permute(1,0,2)

        #out = self.transformer(inpt, e_target, tgt_mask=tgt_mask)
        ...
        return context, concat_qa, q_c, e_ans, e_ans_list
