...
class MHA(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        mask_X = torch.ones(X.size(0), X.size(1), dtype=torch.long)
        mask_Y = torch.ones(Y.size(0), Y.size(1), dtype=torch.long)
...
class BERTCPU(nn.Module):
    def __init__(self, args, vocab, n_dim, image_dim, layers, dropout, num_choice=5):
        ...
        self.util = UtilityLayer(hidden_dim=300, feedforward_dim=600, n_head=10, dropout=0.1)
        self.util2 = UtilityLayer(hidden_dim=300, feedforward_dim=600, n_head=10, dropout=0.1)
    ...
    def _to_one_hot(self, y, n_dims, mask, dtype=torch.FloatTensor):
    ...
    def forward(self, que, answers, **features):
        ...
        #print("ans emb:", e_ans.size())
        ans = answers.view(batch_size * num_options, -1) # reshape for transformer input
        e_ans, _ = self.bert(ans)
        e_ans = e_ans.view(batch_size, num_options, -1, self.bert_dim) # reshape back
        e_ans = e_ans.transpose(0, 1)
        print("ans bert:", e_ans.size())
        ans_len = features['ans_len'].transpose(0, 1)
        e_ans_list = [self.lstm_raw(e_a, ans_len[idx])[0] for idx, e_a in enumerate(e_ans)]

        concat_qa = [(self.get_name(que, q_len) + self.get_name(answers.transpose(0,1)[i], ans_len[i])).type(torch.FloatTensor) for i in range(5)]
        concat_qa_none = [(torch.sum(concat_qa[i], dim=1) == 0).unsqueeze(1).type(torch.FloatTensor) for i in range(5)]
        ...
        if self.script_on:
            ...
            spk_flag = [(spk_flag[i] > 0).type(torch.FloatTensor) for i in range(5)]
        ...
        if self.vmeta_on:
            ...
            vp_flag = [(vp_flag[i] > 0).type(torch.FloatTensor) for i in range(5)]
        ...
        speaker_flag = [(speaker_flag[i] > 0).type(torch.FloatTensor) for i in range(5)]
        ...
        meta_vp_flag = [(meta_vp_flag[i] > 0).type(torch.FloatTensor) for i in range(5)]
        ...
        bb_vp_flag = [(bb_vp_flag[i] > 0).type(torch.FloatTensor) for i in range(5)]
        ...
        concat_s = torch.stack([torch.cat([s_q, s_a[i]], dim=-1) for i in range(5)], 0)
        concat_m = torch.stack([torch.cat([m_q, m_a[i]], dim=-1) for i in range(5)], 0)
        concat_b = torch.stack([torch.cat([b_q, b_a[i]], dim=-1) for i in range(5)], 0)
        ...
        c = self.output(torch.cat([S, M, B, Q], dim=1))
...
class Conv1d(nn.Module):
    ...
    def forward(self, x, x_l):
        # x : (B, T, 5*D)
        x_pad = torch.zeros(x.shape[0],3,x.shape[2]).type(torch.FloatTensor)
