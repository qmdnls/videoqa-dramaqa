...
class MHA(nn.Module):
    ...
    def __init__(self, hidden_dim, pad_size, nhead=8):
        super().__init__()
...
class UtilityBlock(nn.Module):
    ...
    def __init__(self, hidden_dim, feedforward_dim=2048, nhead=5, dropout=0.1):
        ...
        self.multihead_attn = MHA(hidden_dim, pad_size=128, nhead=nhead)
...
class TransformerEncoder(nn.Module):
    def __init__(self, args, vocab, n_dim, image_dim, layers, dropout, num_choice=5):
        ...
        #self.lstm_raw = RNNEncoder(300, 150, bidirectional=True, dropout_p=0.1, n_layers=2, rnn_type="lstm")
        self.transformer_raw_layer = nn.TransformerEncoderLayer(d_model=300, nhead=5)
        self.transformer_raw = nn.TransformerEncoder(self.transformer_raw_layer, num_layers=1)

        #self.lstm_script = RNNEncoder(321, 150, bidirectional=True, dropout_p=0.1, n_layers=2, rnn_type="lstm")
        ...
        self.util1 = UtilityLayer(300)
        #self.util2 = UtilityLayer(300)
        #self.util3 = UtilityLayer(300)
        #self.util4 = UtilityLayer(300)
        ...
        if self.script_on:
            #self.lstm_script = RNNEncoder(321, 150, bidirectional=True, dropout_p=0.1, n_layers=2, rnn_type="lstm")
            self.script_fc = nn.Linear(321, 300)
            self.transformer_script_layer = nn.TransformerEncoderLayer(d_model=300, nhead=5)
            self.transformer_script = nn.TransformerEncoder(self.transformer_script_layer, num_layers=1)
            self.classifier_script = nn.Sequential(nn.Linear(n_dim*2, 1), nn.Softmax(dim=1))
            #self.mhattn_script = CharMatching(4, D, D)

        if self.vmeta_on:            
            #self.lstm_vmeta = RNNEncoder(321, 150, bidirectional=True, dropout_p=0.1, n_layers=2, rnn_type="lstm")
            self.vmeta_fc = nn.Linear(321, 300)
            self.transformer_vmeta_layer = nn.TransformerEncoderLayer(d_model=300, nhead=5)
            self.transformer_vmeta = nn.TransformerEncoder(self.transformer_vmeta_layer, num_layers=1)
            self.classifier_vmeta = nn.Sequential(nn.Linear(n_dim*2, 1), nn.Softmax(dim=1))
            #self.mhattn_vmeta = CharMatching(4, D, D)

        if self.vbb_on:
            #self.lstm_vbb = RNNEncoder(image_dim+21, 150, bidirectional=True, dropout_p=0.1, n_layers=2, rnn_type="lstm")
            self.vbb_input_fc = nn.Linear(image_dim+21, 300)
            self.transformer_vbb_layer = nn.TransformerEncoderLayer(d_model=300, nhead=5)
            self.transformer_vbb = nn.TransformerEncoder(self.transformer_vbb_layer, num_layers=1)
    ...
    def forward(self, que, answers, **features):
        ...
        e_q = e_q.permute(1,0,2) # permute for transformer
        e_q = self.transformer_raw(e_q)
        #e_q = e_q[0,:,:]
        e_q = e_q.permute(1,0,2) # batch first

        # -------------------------------- #
        e_ans = self.embedding(answers).transpose(0, 1)
        ans_len = features['ans_len'].transpose(0, 1)

        #e_ans_list = [self.transformer_raw(e_a, ans_len[idx])[0] for idx, e_a in enumerate(e_ans)]
        e_ans_list = []
        for e_a in e_ans:
            inp = e_a.permute(1,0,2) # permute for transformer
            out = self.transformer_raw(inp)
            #out = out[0,:,:]
            out = out.permute(1,0,2) # permute back to batch first
            e_ans_list.append(out)
        ...
        if self.script_on:
            ...
            #H_S, _ = self.transformer_script(e_s, s_len)
            e_s = self.script_fc(e_s)
            e_s = e_s.permute(1,0,2) # permute for transformer
            H_S = self.transformer_script(e_s)
            H_S = H_S.permute(1,0,2) # batch first
        ...
        if self.vmeta_on:
            ...
            #H_M, _ = self.transformer_vmeta(e_vbe, vmeta_len)
            e_vbe = self.vmeta_fc(e_vbe)
            e_vbe = e_vbe.permute(1,0,2)
            H_M = self.transformer_vmeta(e_vbe)
            H_M = H_M.permute(1,0,2) # batch first
        ...
        if self.vbb_on:
            ...
            vp_flag = [(vp_flag[i] > 0).type(torch.cuda.FloatTensor) for i in range(5)]
            #H_B, _ = self.transformer_vbb(e_vbb, vbb_len)
            e_vbb = self.vbb_input_fc(e_vbb)
            e_vbb = e_vbb.permute(1,0,2)
            H_B = self.transformer_vbb(e_vbb)
            H_B = H_B.permute(1,0,2) # batch first
        ...
        S, M, B, Q = self.util1(S, M, B, Q)
        #S, M, B, Q = self.util2(S, M, B, Q)
        #S, M, B, Q = self.util3(S, M, B, Q)
        #S, M, B, Q = self.util4(S, M, B, Q)

        return S, M, B, Q, concat_qa, q_c, e_ans, e_ans_list
